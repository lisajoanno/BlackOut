...
public class FenetreSimple extends FenetreAbstraite {
	 ...
	 *            the panel
	 */
	public void changeSizePanel(JPanel panel) {
		Font f = Preferences.getData().getCurrentFont();
		// Recherche des composants du panel et modification de leur couleur
		int number = panel.getComponentCount();
		...
		panel.setBackground(Preferences.getData()
	...
	/**
	 * Change color panel and color of all components it contains.
	 * 
	 * @param panel
	 *            the panel
	 */
	public void changeColorPanel(JPanel panel) {
		if (panel != null) {
			// Récuperation des préférences
			Preferences pref = Preferences.getData();

			// Modification de la couleur du panel
			panel.setBackground(pref.getCurrentBackgroundColor());
			...
			for (int i = 0; i < number; i++) {
				...
				c.setBackground(pref.getCurrentBackgroundColor());
				c.setForeground(pref.getCurrentForegroundColor());
